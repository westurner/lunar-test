                    ...
                    for (auto& entry : fs::directory_iterator(p)) {
                        if (entry.is_regular_file() && entry.path().extension() == ".lua") {
                            ...
                            if (g_game && g_game->luaScheduler && g_game->luaScheduler->HasError()) {
                                std::string errorMsg = g_game->luaScheduler->GetLastError();
                                LOGE("Error from LuaScheduler after '%s': %s", entry.path().string().c_str(), errorMsg.c_str());
                                #ifdef LUA_SCHEDULER_HAS_TRACEBACK
                                std::string traceback = g_game->luaScheduler->GetLastTraceback();
                                LOGW("Lua traceback:\n%s", traceback.c_str());
                                #endif
                                scriptError = 1;
                                errorStr = errorMsg;
